...
function Profile() {
  /*
   TODO: 
   - implement profile
   - add possibility to upload photo
   - add authorization
  */
  ...
  return (
    <div className="flex h-full justify-center overflow-hidden text-black dark:text-white">
      ...
      <div className="w-2/3 overflow-scroll pb-10 pt-10 sm:w-10/12 lg:w-2/3">
        <div className="mb-12 text-3xl font-bold">Profile</div>
        <div className="mb-20 flex items-center gap-10">
          <div className="p-2">
            <div className="flex h-24 w-24 items-center justify-center overflow-clip rounded-full ring-1 ring-slate-900 ring-offset-4 ring-offset-white dark:ring-white dark:ring-offset-slate-950">
              <img
          ...
          <div className="flex w-full items-center justify-between">
            ...
            <form
              className="flex flex-col justify-start gap-y-3 text-lg"
            ...
            >
              ...
              <input
                id="profileImg"
                type="file"
                className="file:mr-5 file:cursor-pointer file:rounded-lg file:border-0 file:bg-slate-900 file:px-2 file:py-1 file:text-lg file:text-white file:hover:bg-slate-700 dark:file:bg-white dark:file:text-black dark:file:hover:bg-slate-200"
                {...register('profileImg')}
              />
              <button className="flex w-fit items-center gap-x-3 rounded-lg bg-slate-900 px-3 py-1 uppercase text-white hover:bg-slate-700 dark:bg-white dark:text-black dark:hover:bg-slate-200">
                <HiMiniArrowUpTray />
                <div>upload</div>
              </button>
            </form>
        ...
        <div className="border-b-[1px] border-gray-200 pb-2 text-2xl font-medium dark:border-slate-500">
          My Activity
        </div>
        <div className="my-5 flex justify-start gap-5 text-lg">
          <div
        ...
        {myPostsSelected ? (
          userPosts.length > 0 ? (
            <>
              <div className="mt-10 grid grid-cols-[2fr_1fr_1fr_1fr_1fr] items-center gap-x-5 rounded-t-md bg-slate-900 px-5 py-2 text-lg text-white dark:bg-slate-200 dark:text-black">
                <div>Title</div>
                <div>Date</div>
                <div>Views</div>
                <div>Comments</div>
              ...
              {userPosts?.map((post) => {
                return (
                  <div
                    key={post._id}
                    className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] items-center gap-x-5 border-x-[1px] border-b-[1px] border-gray-200 px-5 py-2 text-lg"
                  >
                    <div>{post.title}</div>
                    <div>
            ...
            <div className="mx-2 mt-10 text-lg">No posts yet</div>
          )
        ) : userComments.length > 0 ? (
          <>
            <div className="mt-10 grid grid-cols-[2fr_2fr_1fr_1fr] items-center gap-x-5 rounded-t-md bg-slate-900 px-5 py-2 text-lg text-white dark:bg-slate-200 dark:text-black">
              <div>Message</div>
              <div>Post</div>
              <div>Date</div>
              <div></div>
            ...
            {userComments?.map((comment) => {
              return (
                <div
                  key={comment._id}
                  className="grid grid-cols-[2fr_2fr_1fr_1fr] items-center gap-x-5 border-x-[1px] border-b-[1px] border-gray-200 px-5 py-2 text-lg"
                >
          ...
          <div className="mx-2 mt-10 text-lg">No comments yet</div>
        )}
      </div>
    </div>
  );
